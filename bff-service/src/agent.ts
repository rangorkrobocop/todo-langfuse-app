...
export const handleAgentAction = async (intent: string, res: Response) => {
    ...
    const createTaskInternal = async (title: string, description: string) => {
    ...
    const updateTaskInternal = async (id: number, updates: any) => {
        const response = await fetch(`${TASKS_SERVICE_URL}/tasks/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
    ...
    const deleteTaskInternal = async (id: number) => {
        const response = await fetch(`${TASKS_SERVICE_URL}/tasks/${id}`, {
            method: 'DELETE'
        });
        return response.json();
    ...
    try {
        ...
        let currentGlobalState = await fetchTasks(false);
        
        // AG-UI: Sync initial state
        sendEvent({ type: 'StateSnapshot', state: currentGlobalState });

        const syncState = async () => {
            const newState = await fetchTasks(false);
                    ...
                    for (const toolCall of functionCalls) {
                        ...
                        if (toolCall.name === 'clearCompletedTasks') {
                            ...
                            if (args && args.confirmed) {
                                const completed = await fetchTasks(true);
                                for (const task of completed) {
                                    await deleteTaskInternal(task.id);
                                }
                            ...
                            const args = toolCall.args as any;
                            if (args && args.title) {
                                await createTaskInternal(args.title, args.description || '');
                            ...
                            if (args && args.id && args.title) {
                                await updateTaskInternal(args.id, { title: args.title, description: args.description || '' });
                            ...
                            const args = toolCall.args as any;
                            if (args && args.id) {
                                await updateTaskInternal(args.id, { completed: true });
                            ...
                            if (args && args.title) {
                                if (args.confirmed) {
                                    ...
                                    const task = all.find((t: any) => t.title === args.title);
                                    if (task) {
                                        await deleteTaskInternal(task.id);
                                        result = `Task "${args.title}" deleted successfully.`;
                                        stateChanged = true;
                                    } else {
                                        result = `Task "${args.title}" not found.`;
                                    }
                                } else {
                                    wasInterrupted = true;
                            ...
                            const currentTasks = await fetchTasks(false);
                            result = `System Data: Current incomplete tasks are ${currentTasks.length}. Titles: ${currentTasks.map((t: any) => t.title).join(', ')}. Please summarize this for the user now.`;
                        }
        ...
        res.write('data: [DONE]\n\n');
        res.end();
        res.destroy(); // Forcefully close the underlying socket immediately
        
        // Flush telemetry in the background so we don't block the UI response
